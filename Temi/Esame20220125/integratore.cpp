...
double Trapezoidi::Calculate(FunzioneBase &f){
    ...
    for(int i=1; i<m_nstep; i++){
        old_int = sum*((m_b-m_a)/(double)i)*getSign();
        sum += f.Eval(getA()+(double)i*getH());
    }
    new_int = sum*getH()*getSign();
    r_prec = ((double)4/(double)3)*fabs(new_int-old_int);
    return sum*getH()*getSign();
}

double Trapezoidi::IntegrateSteps(double a, double b, int n_step, FunzioneBase &f){
    m_nstep = n_step;
    checkInterval(a, b);

...
/* Punto che si muove secondo una legge carina */

PuntoStrano::PuntoStrano() {}
PuntoStrano::~PuntoStrano() {}

void PuntoStrano::setAlpha(double a){
    m_alpha = a;
}

double PuntoStrano::getAlpha() {
    return m_alpha;
}

double PuntoStrano::B_factor(vector<double> &xy){
    return pow(xy[0]*xy[0]+xy[1]*xy[1], m_alpha);
}

vector<double> PuntoStrano::Eval(vector<double> &pos, double t){
    return {pos[2], pos[3], B_factor(pos)*pos[3], -1.*B_factor(pos)*pos[2]};
}